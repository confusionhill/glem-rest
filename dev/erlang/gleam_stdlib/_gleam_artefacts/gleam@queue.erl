...
-opaque queue(DKZ) :: {queue, list(DKZ), list(DKZ)}.

-spec new() -> queue(any()).
new() ->
    {queue, [], []}.

-spec from_list(list(DLC)) -> queue(DLC).
from_list(List) ->
    {queue, [], List}.

-spec to_list(queue(DLF)) -> list(DLF).
to_list(Queue) ->
...
-spec pop_front(queue(DLX)) -> {ok, {DLX, queue(DLX)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(DMC)) -> queue(DMC).
reverse(Queue) ->
...
-spec is_logically_equal(queue(DMK), queue(DMK), fun((DMK, DMK) -> boolean())) -> boolean().
...
-spec is_equal(queue(DMN), queue(DMN)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
